...
class PolicyEngine:
    ...
    def evaluate_rules(
        ...
        """
        results = []
        applicable_rules = self.config.get_rules_for_instruction(instruction_type)
    ...
    def _evaluate_semantic_safety(
        ...
        # Check if this is a Cognitive instruction followed by Execution
        if instruction_type == "TOOL_CALL":
            # Look for recent GENERATE instruction in state history
            if "recent_instructions" in state:
                recent = state["recent_instructions"]
                if recent and recent[-1] == "GENERATE":
                    # Check if this combination is allowed
                    allowed_flows = condition.get("allowed_flows", [])
                    if "GENERATE->TOOL_CALL" not in allowed_flows:
                        return False, {
